...
1. 注释必须解释“为什么这样做”“边界条件是什么”“业务含义是什么”，避免重复代码本身已经表达清楚的内容。
2. 禁止废话注释，例如：
   - `// 定义变量`
   - `// 调用方法`
   - `// 返回结果`
   - `// 遍历列表`
3. 对复杂业务逻辑、关键分支、异常处理、并发控制、事务边界、缓存策略、权限校验、幂等控制、重试机制、降级逻辑、安全校验、性能优化点必须添加注释。
4. 公共方法、接口、类、结构体、枚举、配置项、常量、DTO/VO/BO、数据库模型、消息体、事件对象应添加简洁说明。
5. 对外暴露的 API、RPC、MQ 消费者、定时任务、批处理任务必须说明用途、入参、出参、异常或副作用。
6. 涉及金额、库存、订单、账号、权限、数据一致性、状态流转等核心业务字段时，必须说明业务含义和约束。
7. 临时方案、兼容逻辑、历史遗留逻辑必须标明原因、影响范围和后续处理建议。
8. 注释必须简洁、准确、可维护，不得使用模糊表达，如“这里特殊处理一下”“后面再说”“暂时这样”。
9. 修改代码时，应同步更新过期注释，禁止保留与代码行为不一致的注释。
10. 注释语言统一使用中文，专业术语、协议名、框架名、字段名可保留英文。

### 注释风格

- 类、接口、模块：说明职责和适用场景。
- 方法、函数：说明业务意图、关键参数、返回含义、异常情况。
- 关键代码块：说明设计原因、边界条件或风险点。
- 常量、枚举：说明业务含义，尤其是状态值、类型值、开关值。
- TODO/FIXME：必须包含原因、负责人或处理方向，不允许空泛标记。
